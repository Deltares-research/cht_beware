"""BEWARE runup and flooding calculation model interface.

Provides classes to load BEWARE input files, read/write boundary conditions,
and run the empirical runup/flooding lookup-table approach.
"""
...
class Point:
    ...
    y : float
        Y coordinate.
    name : str, optional
        Human-readable label for this point.
    crs : any, optional
...
class BEWARE:
    """Top-level BEWARE model object.

    Handles loading of input files, boundary point management, and execution
    of the empirical runup/flooding lookup-table approach.

    Parameters
    ----------
    input_file : str, optional
        Path to ``beware.inp``.  When provided the file is read immediately.
    crs : any, optional
        Coordinate reference system descriptor (stored but not currently used).
    """

    def __init__(self, input_file: str = None, crs=None) -> None:
    ...
    def load(self, inputfile: str) -> None:
        """Read the BEWARE input file and all referenced attribute files.
        ...
        self.profiles.read_profile_characteristics(
            file_name=os.path.join(self.path, self.input.profsfile)
        )
    ...
    def run(
        ...
        Hs : array-like, optional
            Significant wave height time series. Defaults to ``self.Hs``.
        Tp : array-like, optional
            Peak wave period time series. Defaults to ``self.Tp``.
        WL : array-like, optional
            Water level time series. Defaults to ``self.WL``.
        betab : array-like, optional
            Beach slope values (not used in this version).
        testprofs : array-like, optional
            Profile IDs to compute. Defaults to all profiles.
        xbFile : str, optional
        ...
        self.out = {
            str(var): np.full((len(testprofs), len(Hs)), np.nan) for var in outvars1
        }
        ...
        for inputprof in range(len(testprofs)):
            ...
            if np.shape(Hs)[0] == 1:
                forcing = np.array(
                    np.concatenate(
                        (Hs[:, inputprof], Tp[:, inputprof], WL[:, inputprof])
                    ),
                    ndmin=2,
                )
            else:
                    ...
                    for iforcings in range(
                        ...
                        for iprof in range(
                            ...
                            if np.isnan(forcing).any():
                                ...
                                intpData[0 : np.shape(BWinds)[1], 0:3] = BWforcing[
                                    BWinds, :
                                ]
                    ...
                    for iforcings in range(
                        ...
                        for iprof in range(
                            ...
                            if np.isnan(forcing).any():
                                ...
                                save["fp_ig"][iforcings, iprof] = np.sum(
                                    np.squeeze(
                                        BWdata[r"obs_25m.fp_ig"][BWinds, profval]
                                    )
                                    * P
                                    * prob[iprof]
                                )
                                save["m0_ig"][iforcings, iprof] = np.sum(
                                    np.squeeze(
                                        BWdata[r"obs_25m.m0_ig"][BWinds, profval]
                                    )
                                    * P
                                    * prob[iprof]
                                )
                                save["setup"][iforcings, iprof] = np.sum(
                                    np.squeeze(
                                        BWdata[r"obs_25m.setup"][BWinds, profval]
                                    )
                                    * P
                                    * prob[iprof]
                                )
    ...
    def read_flow_boundary_points(self) -> None:
        """Populate ``self.flow_boundary_point`` from the loaded profiles."""
        for ind in range(len(self.profiles.xf)):
            name = self.profiles.profid[ind]
            point = BoundaryPoint(
                self.profiles.xf[ind],
    ...
    def read_wave_boundary_points(self) -> None:
        """Populate ``self.wave_boundary_point`` from the loaded profiles."""
        for ind in range(len(self.profiles.xo)):
            name = self.profiles.profid[ind]
            point = BoundaryPoint(
                self.profiles.xo[ind],
    ...
    def write_wave_boundary_conditions(self, path: str = None) -> None:
    ...
    def write_bhs_file(self, file_name: str = None, path: str = None) -> None:
        ...
        file_name : str, optional
            Output path.
        path : str, optional
            Output directory when *file_name* is not specified.
        """
        if not path:
            path = self.path
    ...
    def write_btp_file(self, file_name: str = None, path: str = None) -> None:
    ...
    def read_data(self, input_file: str = None, prcs=None) -> None:
        ...
        prcs : array-like, optional
            List of percentile values to also read (e.g. ``[50, 95]``).
        """
        if not input_file:
        ...
        if prcs is not None:
            for i, v in enumerate(prcs):
                self.R2_prc[str(round(v))] = np.nan_to_num(
                    ds[f"R2_{round(v)}"][:].data, copy=False, nan=0.0
                )
                self.R2_setup_prc[str(round(v))] = np.nan_to_num(
                    ds[f"R2_setup_{round(v)}"][:].data, copy=False, nan=0.0
                )
    ...
    def outline(self, crs=None):
        """Return the outline polygon of the offshore point grid.

        Parameters
        ----------
        crs : any, optional
            Target CRS to project coordinates into.

        Returns
        -------
        xp : list of float
            X coordinates of the outline polygon (closed).
        yp : list of float
            Y coordinates of the outline polygon (closed).
        """
        xg = self.xo
        yg = self.yo
...
class BewareProfiles:
    ...
    def read_profile_characteristics(self, file_name: str = None) -> None:
        ...
        df = pd.read_csv(file_name, index_col=False, sep=r"\s+")
